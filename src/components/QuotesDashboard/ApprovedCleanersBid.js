      ...
      {selectedClient && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
          <div className="bg-white p-6 rounded shadow-lg">
            ...
            <p><strong>Full Name:</strong> {selectedClient.full_name}</p>
            <p><strong>Location:</strong> {selectedClient.address}</p>
            <p><strong>Phone Number:</strong> {selectedClient.phone_number}</p>
            <p><strong>State of Residence:</strong> {selectedClient.state_of_residence}</p>
            
            {/* Add more client details here if needed */}
            <button onClick={closeModal} className="mt-4 bg-red-500 text-white px-4 py-2 rounded block mx-auto">
    Close
  </button>
          </div>
        </div>
